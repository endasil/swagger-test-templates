...
function extractVariablesFromPath(path, pathParams)
{
  var re = new RegExp(/(?:\{+)(.*?(?=\}))(?:\}+)/g);
  var re2;
  var matches = [];
 ...
 */
function pathify(path, pathParams, defaultPathParams) {
  var r;
  ...
  if (Object.keys(pathParams).length > 0) {
    var matches = extractVariablesFromPath(path);

    for (var i = 0; i < matches.length; i++) {
      const match = matches[i];

      let re2 = new RegExp('(\\{+)' + match + '(?=\\})(\\}+)');
